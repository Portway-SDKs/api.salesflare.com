...
class PutWorkflowsIdAudienceRecordIdRequest(BaseModel):
    """
    PutWorkflowsIdAudienceRecordIdRequest
    """ # noqa: E501
    exited: Optional[StrictBool] = Field(default=None, description="Setting `exited` to `true` will exit the entity from the workflow. Setting `exited` to `false` will re-enter the entity into the workflow.")
    met_goal: Optional[StrictBool] = Field(default=None, description="Setting `met_goal` of a workflow record to `false` is currently not possible.")
    __properties: ClassVar[List[str]] = ["exited", "met_goal"]
