...
class GetMe200ResponseRolePermissions(BaseModel):
    """
    GetMe200ResponseRolePermissions
    """ # noqa: E501
    view: Optional[StrictStr] = None
    ced: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["view", "ced"]
    ...
    def view_validate_enum(cls, value):
        ...
        if value not in set(['CED', 'VIEW']):
            raise ValueError("must be one of enum values ('CED', 'VIEW')")
        return value

    @field_validator('ced')
    def ced_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
